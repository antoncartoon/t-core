...
serve(async (req) => {
  ...
  try {
    ...
    if (tokenSymbol !== 'USDC') {
      return new Response(
        JSON.stringify({ error: 'Only USDC minting supported' }),
        { status: 400, headers: corsHeaders }
      );
    }
    ...
    console.log(`TDD minted: ${amount} for user ${user.id}, tx: ${txHash}`);
    ...
    console.error('Error in mint-tdd function:', error);
    return new Response(
