...
/**
 * T-Core Protocol: Piecewise Yield Distribution Function
 * 
 * This implements the new fair and predictable yield curve with 4 distinct tiers:
 * - Safe (0-9): Fixed 5.16% APY
 * - Conservative (10-29): Linear growth 5.16% → 7%
 * - Balanced (30-59): Quadratic growth 7% → 9.5%
 * - Hero (60-99): Exponential growth 9.5% → ~15%
 */
...
// Tier breakpoints and target APYs
...
export const TARGET_APYS = {
  SAFE: 0.0516, // 5.16%
  CONSERVATIVE_END: 0.07, // 7%
  BALANCED_END: 0.095, // 9.5%
  HERO_END: 0.15 // ~15%
};

/**
 * Calculate APY for any segment using the piecewise function
 */
export const calculatePiecewiseAPY = (segment: number): number => {
  ...
  if (i <= TIER_BREAKPOINTS.SAFE_END) {
    // Safe Tier (0-9): Fixed APY
    return TARGET_APYS.SAFE;
  }
  
  if (i <= TIER_BREAKPOINTS.CONSERVATIVE_END) {
    // Conservative Tier (10-29): Linear increase
    ...
    return TARGET_APYS.SAFE + (TARGET_APYS.CONSERVATIVE_END - TARGET_APYS.SAFE) * progress;
  }
  
  if (i <= TIER_BREAKPOINTS.BALANCED_END) {
    // Balanced Tier (30-59): Quadratic increase
    const progress = (i - TIER_BREAKPOINTS.BALANCED_START) / 
                    (TIER_BREAKPOINTS.BALANCED_END - TIER_BREAKPOINTS.BALANCED_START);
    return TARGET_APYS.CONSERVATIVE_END + 
           (TARGET_APYS.BALANCED_END - TARGET_APYS.CONSERVATIVE_END) * Math.pow(progress, 2);
  }
  
  // Hero Tier (60-99): Exponential growth
  const heroProgress = i - TIER_BREAKPOINTS.HERO_START;
  return TARGET_APYS.BALANCED_END * Math.pow(1.03, heroProgress);
};
